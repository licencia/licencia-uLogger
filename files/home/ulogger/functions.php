...
function getFwIpHtml() {
  if ($ip = getFwIp()) {
    return sprintf('<a href="http://%1$s:%2$s">%1$s:%2$s</a> (lösenord: %3$s)', $ip, VNC_PORT, VNC_PASS);
  }
  else {
    return '<span class="label label-important">Fel! Extern IP kunde inte detekteras.</span>';
  }      
}
